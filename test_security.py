# ── test_security.py ─────────────────────────────────────────────
# Security scan to verify no secrets are hard-coded in
# any Python files. All secrets must come from .env file only.
# Required for capstone submission security compliance.
# ────────────────────────────────────────────────────────────────
...
# ── Get actual secret values from .env ──────────────────────────
openai_key   = os.getenv("OPENAI_API_KEY",   "")
jira_token   = os.getenv("JIRA_API_TOKEN",   "")
jira_email   = os.getenv("JIRA_EMAIL",       "")
jira_server  = os.getenv("JIRA_SERVER",      "")

# Use first 8 characters as search patterns
# This avoids printing the full secret in output
secrets_to_check = []

if len(openai_key) > 8:
    secrets_to_check.append({
        "name"   : "OpenAI API Key",
        "pattern": openai_key[:12],
        "critical": True
    })
...
# ── Files to scan ────────────────────────────────────────────────
# Scan all Python files but exclude .env and venv
...
try:
    ...
    if result.stdout.strip():
        ...
        print("  Then commit and push immediately!")
        failed += 1
    else:
...
# ── Check chroma_db is in .gitignore ────────────────────────────
...
try:
    ...
    if "chroma_db" in gitignore_content:
        ...
        print("  Add chroma_db/ to .gitignore")
        warnings += 1
except Exception:
    pass
...
# ── Check os.getenv is used correctly ───────────────────────────
...
# Split patterns so this file does not flag itself
        sk_prefix = "sk" + "-"
        bad_patterns = [
...
if failed == 0 and warnings == 0:
    ...
    print()
    print("No secrets found in any Python or Markdown files.")
    print(".env is protected by .gitignore.")
    print("Your project is safe to submit to GitHub.")
    print()
    print("Ready to move on to Task 54 — Documentation.")

elif failed == 0 and warnings > 0:
    print("Security checks passed with warnings. ✅")
    print()
    print(f"  {warnings} warning(s) found — review above.")
    print("  Warnings are non-critical but worth fixing.")
    print("  Your project is safe to submit.")
    print()
    print("Ready to move on to Task 54 — Documentation.")
    ...
    print()
    print("You MUST fix these before submitting to GitHub:")
    print()
    print("  1. Remove any API keys from Python files")
    print("  2. Make sure .env is in .gitignore")
    print("  3. Run: git rm --cached .env if needed")
    print("  4. Rotate any exposed API keys immediately")
    print("     at platform.openai.com and id.atlassian.com")
print()